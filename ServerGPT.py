...
DEFAULT_CFG = {
    ...
    "file_types": {
        ...
        "Images": {"extensions": ["jpg", "jpeg", "png", "gif", "bmp", "svg", "webp", "tiff", "raw"],
                   "color": "#ec4899"},
        ...
        "Code": {"extensions": ["py", "js", "ts", "jsx", "tsx", "html", "css", "json", "sql", "sh", "bat", "cs", "cpp",
                                "java", "go", "rs", "rb", "php", "swift", "kt"], "color": "#84cc16"},
...
def build_user_msg(fi, preview=""):
    dt = datetime.fromtimestamp(fi["modified"]).strftime("%Y-%m-%d") if fi.get("modified") else "Unknown"
    content_snippet = f"\nContent snippet:\n{preview[:1500]}" if preview else ""

    # Corrected using a triple-quoted f-string to prevent SyntaxErrors
    return f"""Filename: {fi.get('name', '')}
Extension: .{fi.get('type', '')}
Size: {fi.get('size', 0)} bytes
Modified: {dt}
Path: {fi.get('path', '')}{content_snippet}
...
def analyze_file(fi, provider_id, model, system_override=None):
    ...
    try:
        if ptype == "anthropic":
            result = call_anthropic(fi, prov, model)
        elif ptype == "openai":
            result = call_openai(fi, prov, model)
        elif ptype == "ollama":
            result = call_ollama(fi, prov, model)
        else:
            result = call_openai(fi, prov, model)
        return result
    except Exception as e:
...
def do_file_op(src, dst, operation, dry_run, conflict="rename"):
    ...
    if d.exists():
        if conflict == "skip":
            ...
            if d.is_dir():
                return {"status": "error", "error": "Destination is a directory, cannot overwrite",
                        "destination": str(d)}
            # shutil will handle file overwrite automatically
        else:
...
def api_browse():
    ...
    return jsonify(
        {"current": str(p), "parent": str(p.parent) if p.parent != p else "", "dirs": dirs, "drives": drives})
...
def api_stats():
    ...
    s = {
        ...
        "cat_dist": [dict(r) for r in conn.execute(
            "SELECT category,COUNT(*) cnt FROM analysis GROUP BY category ORDER BY cnt DESC").fetchall()],
        "type_dist": [dict(r) for r in conn.execute(
            "SELECT file_type,COUNT(*) cnt FROM files GROUP BY file_type ORDER BY cnt DESC").fetchall()],
        "recent_ops": [dict(r) for r in
                       conn.execute("SELECT * FROM operations ORDER BY executed_at DESC LIMIT 30").fetchall()],
    }
    conn.close()
    return jsonify(s)
